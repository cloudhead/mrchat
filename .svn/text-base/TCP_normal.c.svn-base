...
{
	struct TCP_normal_attr *attr = calloc( 1, sizeof( struct TCP_normal_attr ) );
	char* ip_address = calloc( 16, 1 );
	char *temp_, *temp = calloc( 22, 1 );
	unsigned short port = PORT;

	printf( "TCP_normal_connect:\n" );
...
int TCP_normal_read( struct TCP_normal_attr *attr, struct vector *input, unsigned int maxsize, char opts )
{
	...
	*/
	...
	if( check == -1 )
	{
		...
		{
			case EBADF:
				f_printf( "Invalid File Descriptor" );
				break;
			case EINTR:
				f_printf( "A signal was caught" );
				break;
			case ENOMEM:
				f_printf( "Unabe to allocate fuckign memory" );
				break;
			default:
				break;
		}
    ...
    if( i > 0 )
    {
        ...
        #ifdef DEBUG
        	d_printf( "TCP_normal_read: Data recieved:", input->buf, i );
        #endif
    }
...
int TCP_normal_update_stat( char** stat, void *pattr, int size )
{
	struct TCP_normal_attr *attr = pattr;
